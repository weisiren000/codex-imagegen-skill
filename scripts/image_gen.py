...
def _load_config() -> Dict[str, Any]:
    ...
    try:
        ...
        _warn(f"Failed to read imagegen config.json: {exc}")
        return {}
    if not isinstance(data, dict):
        _warn("imagegen config.json must contain a JSON object.")
        return {}
    return data


def _config_key(config: Dict[str, Any]) -> Optional[str]:
    value = config.get("key", config.get("api_key"))
    return str(value).strip() if value else None


def _config_base_url(config: Dict[str, Any]) -> Optional[str]:
    value = config.get("url", config.get("base_url"))
    if not value:
        return None
    ...
    if not re.match(r"^https?://", url, flags=re.IGNORECASE):
        _warn("config.json url/base_url is missing http(s)://; prepending https://")
        url = f"https://{url}"
    return url


def _config_model(config: Dict[str, Any]) -> Optional[str]:
    value = config.get("model")
    return str(value).strip() if value else None


def _effective_api_key(config: Dict[str, Any]) -> Optional[str]:
    return os.getenv("OPENAI_API_KEY") or _config_key(config)


def _effective_base_url(config: Dict[str, Any]) -> Optional[str]:
    return os.getenv("OPENAI_BASE_URL") or _config_base_url(config)


def _effective_model(config: Dict[str, Any], explicit_model: Optional[str]) -> str:
    return (
        explicit_model
        or os.getenv("OPENAI_IMAGE_MODEL")
        or _config_model(config)
        or DEFAULT_MODEL
    )
...
def _ensure_api_key(dry_run: bool, config: Dict[str, Any]) -> None:
    if _effective_api_key(config):
        print("OPENAI_API_KEY is set.", file=sys.stderr)
        return
    if dry_run:
        _warn("No API key found in OPENAI_API_KEY or imagegen config.json; dry-run only.")
        return
    _die("No API key found. Set OPENAI_API_KEY or add key/api_key to imagegen config.json.")
...
def _create_client():
    ...
    config = _load_config()
    client_kwargs: Dict[str, Any] = {"api_key": _effective_api_key(config)}
...
def main() -> int:
    ...
    config = _load_config()
    args.model = _effective_model(config, args.model)
    ...
    _ensure_api_key(args.dry_run, config)
